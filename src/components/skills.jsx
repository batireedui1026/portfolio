const Skills = () => {
  return (
    <div>
      <p className=" bg-teal-300 w-24 rounded mt-52  text-center ml-[1050px]">
        skills
      </p>
      <br />
      <p className="text-center text-gray-400">
        The skills, tools and technologies I am really good at:
      </p>
      <br />
      <br />
      <br />
      <div className="flex gap-40 justify-center">
        <img
          className="w-16 h-16"
          src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/99/Unofficial_JavaScript_logo_2.svg/1200px-Unofficial_JavaScript_logo_2.svg.png"
        />
        <img
          className="w-16 h-16"
          src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/4c/Typescript_logo_2020.svg/1200px-Typescript_logo_2020.svg.png"
        />
        <img
          className="w-16 h-16"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQcR5U16C8yXgBpl7-Bc7Itjx3_LRl425zINA&s"
        />
        <img
          className="w-16 h-16"
          src="https://www.drupal.org/files/project-images/nextjs-icon-dark-background.png"
        />
        <img
          className="w-16 h-16"
          src="https://static-00.iconduck.com/assets.00/node-js-icon-454x512-nztofx17.png"
        />
        <img
          className="w-16 h-16"
          src="https://external-preview.redd.it/learn-express-js-a-beginner-friendly-step-by-step-guide-v0-hE7Pn4a6ZRCe82cUQnRhXPmvzcp12NG5m8qnWsOpsGI.jpg?auto=webp&s=ac7405a8046d97bae600b10cf191279e612984e7"
        />
        <img className="w-16 h-16" src="https://nestjs.com/img/nest-og.png" />
        <img
          className="w-16 h-16"
          src="https://upload.wikimedia.org/wikipedia/commons/9/96/Socket-io.svg"
        />
      </div>
      <br />
      <br />
      <br />
      <div className="flex gap-40 justify-center">
        <img
          className="w-16 h-16"
          src="https://btech.id/media/images/Page/2023/04/05/postgresql-logo-1-350x220-90992449"
        />
        <img
          className="w-16 h-16"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTrjABweqT7gL00cuGom4aYvIzD9KINiaj-iA&s"
        />
        <img
          className="w-16 h-16"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR01L6Jq6BvPIjmhTliTvigg2nAADj_Z-cfPw&s"
        />
        <img
          className="w-16 h-16"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQNhoXisDruJMDAq3Ltd-wuaMW2lGxck9wAKw&s"
        />
        <img
          className="w-16 h-16"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSoUX5LMRa7atIsNfl0nP3DaUaV4URhV0PHfA&s"
        />
        <img
          className="w-16 h-16"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRM_9OMVnolrYxanxA7JfY8VOwceeffcbgL9A&s"
        />
        <img
          className="w-16 h-16"
          src="https://static-00.iconduck.com/assets.00/storybook-icon-icon-412x512-341bo8r1.png"
        />
        <img
          className="w-16 h-16"
          src="https://logowik.com/content/uploads/images/git6963.jpg"
        />
      </div>
    </div>
  );
};
export default Skills;
